...
;;(defun hide-stream-in-data-structure     (stream) (list stream))
(defun hide-stream-in-data-structure     (stream) stream)
;;(defun reveal-stream-from-data-structure (stream) (car  stream))
(defun reveal-stream-from-data-structure (stream) stream)
;;(defun hidden-stream-data-structure-p (stream) (litsp stream))
(defun hidden-stream-data-structure-p (stream) (atom stream))
...
(defun lisp-system-independent-process-revoke-arrest-reason (process reason)
  #+:allegro
  (apply #'mp:process-revoke-arrest-reason process (print (if (null reason) '("foo") reason)))
  #+:lispworks
...
(defun tk-open (&optional (interpreter (uw-wish-location))
			  ...
			  (load-wish-inits t)           ;; should default to nil but leaving calls unchanged for now
			  ;; (start-stream-listener t)  ;; should have (and should default to nil) but doing minimal disruption for now
			  )
  (let* ((stream 
	  #+:allegro
	  (if (typep interpreter 'file-simple-stream)
	      interpreter           ;; special case where where we've already open a file
	    (hide-stream-in-data-structure (open-wish-socket)))    ;; tmp clude to check whether derek is doing anything stupid with the stream
          #+:cmu
	 ...
	 (stream-listener (if (typep (reveal-stream-from-data-structure stream) 'file-simple-stream)
			      'no-get-stream-as-stream-is-file-opened-for-output
...
(defun tk-put (stream command &rest command-string-args)
  ...
  (tell-wish (reveal-stream-from-data-structure stream) command)
  command)

(defun tk-get (stream)
  (if (numberp stream)
      (setq stream (assoc-value-1 stream *tk-streams*)))
  (wish-read (reveal-stream-from-data-structure stream)))

(defun tk-close (stream)
  (if (or (null stream) (not (or (numberp stream) (streamp (reveal-stream-from-data-structure stream)))))
      (error "please pass a stream number or stream to tk-close, you passed ~d" stream))
  (if (and (hidden-stream-data-structure-p stream) (streamp (reveal-stream-from-data-structure stream)))
      (setq stream 
  ...
  (if (numberp stream)
	...
	(if (open-stream-p (reveal-stream-from-data-structure stream))  ;; hack to get around writing the tcl to file, and closing twice, once for table window, and once for the mds window ugly because really should do sepeartely for each window 20040913, can remove when we remove the ugly hack for stream to tcl file
	    (progn
	      ...
	      (close-wish-socket (reveal-stream-from-data-structure stream)) 
	      #+:allegro
	      (if (not (typep (reveal-stream-from-data-structure stream) 'file-simple-stream))
		  (system:os-wait))
	      (if *last-background-process*
		  (progn
		    (lisp-system-independent-process-revoke-arrest-reason 
		     *last-background-process*
		     (car (mp:process-arrest-reasons *last-background-process*)))
		    (mp:process-kill
		     *last-background-process*)
		    (setq *last-background-process* nil)))
	      (close (reveal-stream-from-data-structure stream))
	      (if (not (typep (reveal-stream-from-data-structure stream) 'file-simple-stream))
		  (mp:process-kill tk-interpreter)))))))
			 ...
			 (print "No process to stop")))
		      ((and (listp command) (member (car command) background-commands))
		       ...
		       (if (and *last-background-process*
				(mp:process-arrest-reasons *last-background-process*))
			   (lisp-system-independent-process-revoke-arrest-reason 
			    *last-background-process*
			    (car (mp:process-arrest-reasons *last-background-process*)))
