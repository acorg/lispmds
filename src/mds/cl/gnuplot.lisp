...
(defun derive-x-y-sizes (x-size y-size ratio)
  (if ratio
	...
	(if x-size
	  ...
	  (if y-size
	      (values (float (/ y-size ratio)) y-size ratio)
	    (values 1.0 (/ 1.0 ratio) ratio))))
    (if (and x-size y-size)  ;; and (null ratio) because of the above
	(values
	 x-size
	 y-size
	 (float (float (/ y-size x-size))))
    (values
     x-size
     y-size
     ratio))))
...
(defun gnuplot-label-format (label)
  (let ((text (nth 0 label))
        (x    (nth 1 label))
        (y    (nth 2 label))
        (rotate     (snoop-keyword-arg :rotate     label :not-found-action :return-nil))         
        (font-size  (snoop-keyword-arg :font-size  label :not-found-action :return-nil))
        (label-font (snoop-keyword-arg :label-font label :not-found-action :return-nil)))
    (format nil "set label ~s at ~d,~d ~a ~a"
            text
            x 
            y
            (if rotate "rotate" "")
            (if (or label-font font-size) 
                (format nil "font \"~a,~d\"" 
...
(defun gnuplot-basic (coords &key ;;(style 'line)   
			    (refresh t)
			    (element-name "")
			    ;;(element-color (random-x-color))  ;; set by element-linetype below
			    element-symbol     ;; NOTE: element-pointype sets the symbol.  "test" in gnuplot, shows the style choices
			    element-dashes
			    (element-style 'lines)  ;; yerr, with :bar 'small and :element-pointtype 2 is good, 'points gives scatterplot
                                                    ...
                                                    ;; impulses is for impulses 
			    element-linewidth
			    element-linetype   ;; do test in gnuplot to see the types 
                                               ;; (gnuplots (loop for i below 20 collect '((0 0) (1 0))) :element-name "foo") ;; superseded below
                                               ;; 1-8 changes color, then repeats in X11 (1=red, 2=green, 3=blue, 4=violet, 5=cyan, 6=brown, 7=red-orange, 8=orange)
                                               ;; linetype -1 is always black, 0 is black dashed
                                               ;; calling test in gnuplot shows what is available for the terminal type
			    ...
			    x-tics y-tics z-tics  ;;can be label-position pairs or just labels (just labels does not seem to work 2013-01)
			    y2-tics
			    x-tic-format y-tic-format z-tic-format
                            no-x-tics no-y-tics no-z-tics
			    tic-direction ;;in or out
			    ;;latex
			    hardcopy-only         ;; note, also pass ps-filename if passing hardcopy-only, and pass
                            hardcopy-fontsize     ;;   hardcopy-ony, hardcopy-fontsize, ps-filename, and size to all gnuplots when :refresh nil
			    (ps-filename "/Users/dsmith/misc/ps/xy.ps")  ;;   after the first call
			    legend-position
			    ;;(legend-mapped t)
			    ;;big-font
			    ;;tag
			    label ;;(text x y)
			    labels
                            size      ;; do not pass size if also passing x-size or y-size.
			    x-size
			    y-size
  ...
  (if (and size (or x-size y-size))
      (progn
        (print "Do not specify size if also specifying x-size or y-size, removing size")
        (setq size nil)))

  (if size
      (setq x-size (if ratio nil (* size 1.41))  ;; if ratio passed, then will be set in derive-x-y-sizes
            y-size size))
  ...
  (let ((width  (round (* (or x-size 1) (* *gnuplot-internal-screen-ratio* *unit-y-size-in-pixels* window-scale))))
    ...
    (if refresh (setq *gnuplot-stream* (fork-and-pipe "gnuplot"))) ;;"gnuplot -geometry ~dx~d" width height))))
  ...
  (if hardcopy-only (gnuplot-command "set te postscript eps color ~a" 
                                     (if (numberp hardcopy-fontsize) hardcopy-fontsize ""))) 
  ...
  (if no-x-tics (gnuplot-command "unset xtics"))
  (if no-y-tics (gnuplot-command "unset ytics"))
  (if no-z-tics (gnuplot-command "unset ztics"))

  (if tic-direction (gnuplot-command "set tics ~a" (string-downcase tic-direction)))
  
  (if label (gnuplot-command (gnuplot-label-format label)))
  (if labels (loop for label in labels do (gnuplot-command (gnuplot-label-format label))))
  
  (if title (gnuplot-command "set title ~s" title))

  (if x-title (gnuplot-command "set xlabel ~s" x-title))   
  ...
  (if refresh
	...
	(setq *last-gnuplot-plot-number* (krandom 1000000000 *unique-seed-each-restart-rng*)))
    (progn
      (if next-multiplot
	  (progn
	    (setq *last-gnuplot-plot-command* (if 3d-plot "sp " "p "))
  ...
  ;;(if hardcopy-only
  ;;    (fix-trailing-a-error-in-directly-generated-ps-file ps-filename))
  )

(defun fix-trailing-a-error-in-directly-generated-ps-file (ps-filename)
  (let ((file-as-strings (fi-in-readline ps-filename)))
    (if (equal "a" (car (last file-as-strings)))
    (fi
     (butlast file-as-strings)
     ps-filename
     :supersede
     t
...
(defun gnuplot-correlation (pairs &rest plot-args)
  (let ((x-min (or (and (member :x-min plot-args) (snoop-keyword-arg :x-min plot-args))
    ...
    (let (;; extract the plot-args that are specific for doing correlation, use them here, and do not pass them on to sub gnuplot functions
          (short-correlation-names      (snoop-keyword-arg :short-correlation-names      plot-args :not-found-action :return-nil))
	  ...
	  (equal-axes-range             (snoop-keyword-arg :equal-axes-range             plot-args :not-found-action :return-nil))
	  (plot-args (remove-keywords-and-args
          ...
          ;; extract plot args specific for making hardcopy only and pass to sub gnuplot functions
          (hardcopy-only     (snoop-keyword-arg :hardcopy-only     plot-args :not-found-action :return-nil))
          (ps-filename       (snoop-keyword-arg :ps-filename       plot-args :not-found-action :return-nil))
          (hardcopy-fontsize (snoop-keyword-arg :hardcopy-fontsize plot-args :not-found-action :return-nil))
          (size              (snoop-keyword-arg :size              plot-args :not-found-action :return-nil)))

      (if equal-axes-range
	  (progn
	    (setq x-min (floor (min x-min y-min)))
	    (setq y-min (floor (min x-min y-min)))
	    (setq x-max (ceiling (max x-max y-max)))
	    (setq y-max (ceiling (max x-max y-max)))))
	...
	(apply #'gnuplot 
	       ...
	       :element-name (if show-correlation-coefficient
				 (format nil "~a~5,2f" 
					 (if short-correlation-names "r=" "Correlation r=")
					 r)
			       "")
	       ;;:label
		...
		;;	     r)
		 ;;    (+ x-min (* 0.1 (- x-max x-min)))
		  ;;   (- y-max (* 0.1 (- y-max y-min))))
	       :ratio 1
	       (append 
		(if (snoop-keyword-arg :size plot-args :not-found-action :return-nil)
		    nil
		  (list :size 1.0))
		plot-args
                (if equal-axes-range
	...
	(gnuplot-regression-line 
         ...
         :short-correlation-names short-correlation-names
...
(defun gnuplot-regression-line (m c x-min x-max y-min y-max &optional &key
								      ...
								      (refresh t))
  (if (zerop m)
      (format nil "~2%Slope zero passed to gnuplot-regresion-line.  No regression line plotted")
...
(defun gnuplot-distribution (ys &rest plot-and-other-args)
  (let* ((round-to-nearest (if (snoop-keyword-arg :round-to-nearest plot-and-other-args :not-found-action :return-nil)
	...
	(setq plot-args (append plot-args '(:ratio 1.0))))
...
(defun gnuplot-png (&optional fontsize (filename "/home/dsmith/misc/ps/xy.ps") (size 1.0) 
  ...
  ;; and the same size (eps 1/2s the size), but we need to make 0.75 size, so smaller gifs, 
  ;; and to rotate back to portrait with the mogrify
  (gnuplot-hardcopy fontsize filename size "landscape")
  (sleep sleep-amount-after-gnuplot-hardcopy)
  ;; uncomment here to make solid lines
  (progn
    (run-shell-command (format nil "mv -f ~a ~a.old" filename filename))
    (run-shell-command (format nil "perl -pe \"s/Solid false/Solid true/\" ~a.old > ~a" filename filename)))
  (sleep sleep-amount-after-making-lines-solid)
...
(defun gnuplot-jpg (&optional fontsize (filename "/home/dsmith/misc/ps/xy.ps"))
  ...
  (gnuplot-hardcopy fontsize filename 0.75 "landscape")
  (sleep 1)
  (run-shell-command-and-wait (format nil "mogrify -rotate 90 -format jpg ~a" filename)))
...
(defun gnuplot-exit ()
  ...
  ;;(close-wish-socket *gnuplot-stream*)
  )
