...
(defun fork-and-pipe (command)
  (let* ((stream
	  #-:openmcl
	  (run-shell-command (string-append "exec " command)
			     :wait nil 
			     :input :stream 
			     :output :stream 
			     :error-output :output)
...
(defun gnuplot-basic (coords &key ;;(style 'line)   
  ...
  (let ((width  (round (* (or x-size 1) (* *gnuplot-internal-screen-ratio* *unit-y-size-in-pixels* window-scale))))
    ...
    (if refresh (setq *gnuplot-stream* (fork-and-pipe "/sw/bin/gnuplot")))
    )
  ...
  (gnuplot-fi (ll-to-lines 
		 ...
		 coords))
	      :supersede ;;:new-version  djs, 25oct2003, changed for openmcl
