...
(defun canvas-to-mds-coords (mds-window canvas-coords)
  ...
  (let ((mds-coords 
    ...
    (if t ;;*basis-vector-point-indices*
...
(defun visualize-mds-coordss (tk new-or-update? coordss coords-names coords-colors stress 
			      &key title
				   ...
				   (coords-dot-sizes  (loop for n in coords-names collect (progn n 4)))
				   ...
				   (coords-outline-colors (if (get-table-window-for-mds-window tk)
							    ...
							    t))
				   (coords-shapes (if (get-table-window-for-mds-window tk)
						      (get-coords-shapes (get-table-window-for-mds-window tk))))
				   ;; default for coords-shapes is also set further down
				   )
	  ...
	  (case new-or-update?
	    (new 
	     (let ((item 1)
	       ...
	       (tk-put tk "mkText ~a 10 25 stress sw black ~a" (if (numberp stress) (dps stress 4) stress) (tk-font 11))
	       (setq item (+ item 1))
		   ...
		   for i from 0 do
		     ...
		     (tk-put tk "set itemColor(~d) ~a" item coords-color)
		     (setq item (+ item 1))
		     (tk-put tk "mkText ~s ~{~d ~} strainName se ~a ~a" ;;to get only names, make anchor center 
		   ...
		   (let ((length (if (listp show-scale-with-length)
		     ...
		     (let ((mds-length (hypotenuse-length (mds-to-canvas-coords tk (list 0 0))
		       ...
		       (loop for x from 0 to (inc (* mds-tic-interval (get-num-vertical-grid-lines tk))) by mds-tic-interval do
			     (tk-put tk "mkColoredLine ~d 30 ~d ~d grid #eeeeee" x x (* 2 (get-canvas-height tk)))
			     (setq item (+ item 1)))
		       (loop for y from 30 to (inc (* mds-tic-interval (get-num-horizontal-grid-lines tk))) by mds-tic-interval do
			     (tk-put tk "mkColoredLine 0 ~d ~d ~d grid #eeeeee" y (* 2 (get-canvas-width tk)) y)  
			     (setq item (+ item 1)))
		       )))
	    ...
	    (update 
	     (let ((item 1))
	       (if stress (tk-put tk "setTextText ~d ~a" item (if (numberp stress) (dps stress 4) stress)))
	       (setq item (+ item 1))
		   ...
		   for i from 0 do
		     ...
		     ;; set coords color
		     (if (not (equal coords-color old-coords-color))
		     ...
		     (setq item (+ item 1))
		     (tk-put tk "setTextXY ~d ~{~d ~}" item (mds-to-canvas-coords tk coords))
	    ...
	    (update-changes-only 
;;	     (error "Called visualize-mds-coords with update-changes-only, that was not changed to update presumably because none of the basis vectors changed.  Hack for now
	     (let ((item 1))
	       (if stress (tk-put tk "setTextText ~d ~a" item (if (numberp stress) (dps stress 4) stress)))
	       (setq item (+ item 1))
	    ...
	    (t (error "end of case"))))
	;; (set-mds-coordss tk coordss))   ;; why is this here?!!!!!!
  ...
  (let* ((actual-distance (e-dist from-coords to-coords))
	 (target-distance (if (true-dont-care-p target-value)
				...
				(if (thresholdp target-value)
				    ...
				    (adjust-table-value (threshold-number target-value) target-col-base target-row-adjust)
...
(defun coords-on-way-to-point (distance-to-move from-point to-point &optional distance-between-points)
  ...
  (if (and (zerop distance-between-points)
    ...
    (let ((sin (/ (- (nth 1 to-point) (nth 1 from-point))
		  distance-between-points))
	  (cos (/ (- (nth 0 to-point) (nth 0 from-point))
		  distance-between-points)))
      (list 
       (+ (nth 0 from-point) (* distance-to-move cos))
       (+ (nth 1 from-point) (* distance-to-move sin))))))
...
(defun calc-error-coords-and-class (make-or-update tk coordss coordss-more &optional &key item selected-subset)
  ...
  (let* ((show-error-lines (get-show-error-lines (get-table-window-for-mds-window tk)))
	...
	(let* ((similarity-table-p (similarity-table-p (get-hi-table (get-table-window-for-mds-window tk))))
      ...
      (loop for i from 0 for i-coords in coordss for components-row in components do
	    (loop for j from 0 for j-coords in coordss for (working-target-table-value   
							    original-target-table-value   
							    original-target-table-value-is-true-dont-care
							    working-target-table-value-is-true-dont-care
							    stress-component
							    stress-class) in components-row do
				...
				(if (if prediction-lines-only
				    ...
				    (let ((actual-distance (e-dist i-coords j-coords)))
					  ...
					  (if (< actual-distance 
						 ...
						 (adjust-table-value (threshold-number working-target-table-value) 
								     (nth (max i j) col-bases) 
								     (nth (min i j) row-adjusts)))
					      (values
			  ...
			  (let ((canvas-error-coords
				 (flatten 
				  (mapcar (^ (coords) (mds-to-canvas-coords tk coords))
					  (list
					   (if (equal '(-1234567 -1234567) error-coords)
					       error-coords
					     i-coords)
					   error-coords))))
...
(defun update-error-lines (tk coordss coordss-more item &optional &key selected-subset)
  (let ((saved-error-lines-selected-subset (get-error-lines-selected-subset tk)))
	...
	(if (not (equal selected-subset saved-error-lines-selected-subset))
	    (format t "assumption violatoin in update-error-lines"))
...
(defvar *last-point-stresses*)
(setq *last-point-stresses* 'not-set)

;;(defvar *color-ag-sr-differently*)
;;(setq *color-ag-sr-differently* t)
;;(setq *color-ag-sr-differently* nil)
...
(defun set-point-colors-based-on-stress (mds-window coordss more &key (color-ag-sr-differently 'not-set) average-stress)
  ...
  (let* ((point-stresses 
	 ...
	 (point-stresses-in-range-0-to-1 (scale-zero-based-data-so-max-is-1 point-stresses))  ;;(put-in-range-0-to-1 point-stresses))
	 (antigens (hi-table-antigens (get-hi-table-working-copy (get-table-window-for-mds-window mds-window)))))

    (setq *last-point-stresses*
      (transpose 
       antigens
       point-stresses))
	...
	(let ((ag-point-stresses-in-range-0-to-1 (put-in-range-0-to-1 ag-point-stresses))
	      (sr-point-stresses-in-range-0-to-1 (put-in-range-0-to-1 sr-point-stresses)))
	  (loop for name in antigens
	      for canvas-item from (get-mds-window-first-coords-canvas-item mds-window) by 2 collect
		(tk-put mds-window "setIdFill ~d ~a" 
			canvas-item
...
(defun raise-strains-from-strain-names (mds-window strain-names)
  ...
  (with-open-file (out filename :direction :output 
    ...
    (let ((hi-table (get-hi-table (get-table-window-for-mds-window mds-window)))
      ...
      (format out ";; MDS distances for ~a~%;; This file created on ~a~%~%" 
	      ...
	      (time-and-date))
      (loop for (a-name . rest-names) on (hi-table-antigens hi-table) for (a-coords . rest-coordss) on coordss do
	    (loop for b-name in rest-names for b-coords in rest-coordss do
		  (format out "~{~16a ~}~10f~%" (sort-strains (list a-name b-name)) (dps (e-dist a-coords b-coords) 6)))))))
...
(defun output-distance-matrix (mds-window filename)
  (with-open-file (out filename :direction :output 
		   :if-exists :supersede
		   :if-does-not-exist :create)
    (format out ";; MDS distance matrix for ~a~%;; This file created on ~a~%~%" 
	    (hi-table-name (get-hi-table (get-table-window-for-mds-window mds-window)))
	    (time-and-date))
...
(defun output-phylip-matrix (table coordss filename &key 
  ...
  (with-open-file (out filename :direction :output 
    ...
    (fll (phylip-table table coordss :normalize-to-max-distance-of-one normalize-to-max-distance-of-one) 
	 :stream out)))
...
(defun phylip-table (table coords &key
  ...
  (let* ((names (mapcar (^ (name-with-ag-sr) 
	 ...
	 (distance-matrix (coordss-to-full-distance-matrix (firstn (length names) coords) #'e-dist)))
...
(defun output-names-and-coordss-from-mds-window (mds-window filename point-canvas-ids)  ;; null point-canvas-ids means no points selected, so output all names
  ...
  (let* ((table-window (get-table-window-for-mds-window mds-window))
    ...
    (output-names-and-coordss
     ...
     (multiple-nth point-indices coordss-plus-more))))
...
(defun output-table-extract (mds-window filename point-canvas-ids &optional &key include-all-sera exclude-all-sera)
  ...
  (with-open-file (out filename :direction :output 
    ...
    (let* ((table-window (get-table-window-for-mds-window mds-window))
	    ...
	    (let ((selected-point-indices
		  ...
		  (additional-point-indices
		   (if include-all-sera
			   ...
			   (error "Only ask to include all sera for an HI table in raw (no pannel) format")
			 (series (length (hi-table-antigens-short table)) (dec (hi-table-length table))))
		     nil))
		  (excluded-point-indices
		   (if exclude-all-sera
		       (if (not (ag-sr-table-p table))
			   (error "Only ask to exclude all sera for an HI table in raw (no pannel) format")
			 (series (length (hi-table-antigens-short table)) (dec (hi-table-length table))))
		     nil)))
      ...
      (format out 
	      ";; MDS configuration file (version 0.4).~%;; Created for ~a at ~a~%~%"
	      ;; note, the above line is also in save-configuration below, if we change it here, change it there and vice-versa.
	      ;; 0.4 is a change in plot spec to record shapes
      	      (hi-table-name table)
      	      (time-and-date))
      (format out "~s" (make-save-form :hi-table table-extract
				       ...
				       :coords-shapes (multiple-nth point-indices (get-coords-shapes table-window))
      ...
      (let ((mds-window
	     (nth-value 1 
			(mds-hi-table (get-table-window-for-mds-window mds-window)
				      ...
				      (if new-window? (get-canvas-coord-transformations mds-window)) ;; copy from seeding window
...
(defun tk-image-convert-ps-to-gif (filename)
  (if (not (running-on-windows-p))
      (run-shell-command (format nil "mogrify -format gif ~a" filename))))  ;; convert to gif format (from ps), netscape does not read it, but xv does

(defun tk-image-convert-ps-to-png (filename)
  ;; png works in IE, and is ok in netscape4 (but not great), and has about the same size on map files as a jpg
  (if (not (running-on-windows-p))
      (run-shell-command (format nil "mogrify -format png ~a" filename))))
...
(defun png-from-canvas (tk filename-unpostfixed)
  (tk-put tk ".c postscript -file ~a.ps" filename-unpostfixed)
  (sleep 1)
  (tk-image-convert-ps-to-png (format nil "~a.ps" filename-unpostfixed))
  (sleep 1))
