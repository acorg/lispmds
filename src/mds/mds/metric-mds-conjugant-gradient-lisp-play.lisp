  ...
  (apply #'mds
	 (setq *metric-conjugant-gradient-stress-f* 
	   (^ (coordss &optional old-coordss (initial-or-next 'initial))   ;; initial or next is a bit of an ugly hack for 
	      ...
	      (setq numerical-recipies::D target-dists)
	      (setq numerical-recipies::NUM_POINTS (length coordss))
	      (setq numerical-recipies::NUM_DIMS (length (car coordss)))
	      (setq numerical-recipies::MOVEABLE_COORDS 
